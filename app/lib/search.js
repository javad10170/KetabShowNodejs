...
const RESULT_REGEX_ec = /[0-9]+\ files\ found/i;
const RESULT_REGEX_io = /[0-9]+\ books\ found/i;
...
function idFetch(options,callback){
  ...
  request(httpOptions, (err, response, body) => {
    ...
    let results = body.match(options.mirror == "http://libgen.io" ? RESULT_REGEX_io : RESULT_REGEX_ec);
    if (results === null)
